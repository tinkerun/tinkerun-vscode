 ...
 */
export function spawn (file: string, args: string[] | string, options: IPtyForkOptions | IWindowsPtyForkOptions): IPty|undefined {
  const pty = getCoreNodeModule('node-pty')
  if (pty != null) {
...
export interface IPty {
   ...
   */
  on: ((event: 'data', listener: (data: string) => void) => void) & ((event: 'exit', listener: (exitCode: number, signal?: number) => void) => void)
